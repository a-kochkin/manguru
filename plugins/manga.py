...
def is_manga_in_dict(_, client: Bot, message: Message):
...
async def get_buttons_for_chapters(manga: MangaCard, user_id: int, page: int):
    ...
    if prev:
        footer.append(InlineKeyboardButton('<<', callback_data=f'pagination_manga_{manga.unique()}_{prev}'))
    if next_:
        footer.append(InlineKeyboardButton('>>', callback_data=f'pagination_manga_{manga.unique()}_{next_}'))
...
@Bot.on_message(filters.create(is_manga_in_dict))
...
@Bot.on_callback_query(filters.regex(r'^pagination_manga_'))
async def on_callback_query(client, callback: CallbackQuery):
    manga = mangas[callback.data.split('_')[2]]
